...
if uploaded_file is not None and models_ready:
    ...
    if np.sum(preds_p) > 1.05 or np.sum(preds_p) < 0.95: preds_p = tf.nn.softmax(preds_p).numpy()
    ...
    if np.sum(preds_s) > 1.05 or np.sum(preds_s) < 0.95: preds_s = tf.nn.softmax(preds_s).numpy()
    ...
    if np.sum(preds_c) > 1.05 or np.sum(preds_c) < 0.95: preds_c = tf.nn.softmax(preds_c).numpy()
    ...
    if "ऑटो" in crop_mode and white_trails > 0.05 and lesion_ratio < 0.02:
        is_out_of_scope = True

    if is_out_of_scope:
        with col_right:
            st.markdown('<div class="oos-card"><div class="oos-tag">⚠️ अनोळखी पीक / OUT OF SCOPE PLANT</div><h3 style="color:#991B1B; margin:6px 0;">हे पान अधिकृत ३ पिकांमधील नाही!</h3><p style="color:#374151; font-size:0.95rem;">हे पान टोमॅटो किंवा इतर वनस्पतीचे दिसते. कृषी-AI सध्या केवळ बटाटा, कापूस आणि सोयाबीन या ३ पिकांसाठी प्रमाणित आहे.</p></div>', unsafe_allow_html=True)
    else:
        ...
        if "बटाटा" in crop_mode:
            ...
            # Gemini Vision API: फोटो पाहून आधी पिकाची अचूक पडताळणी करेल
            if gemini_client:
                try:
                    gem_check = gemini_client.models.generate_content(
                        model="gemini-2.5-flash",
                        contents=[
                            "Examine this agricultural plant leaf carefully. Identify whether it is 'cotton', 'potato', or 'soybean'. Answer with ONLY ONE SINGLE WORD: cotton, potato, or soybean.",
                            img
                        ]
                    )
                    g_ans = gem_check.text.strip().lower()
                    if "cotton" in g_ans:
                        selected_crop = "cotton"
                    elif "soybean" in g_ans:
                        selected_crop = "soybean"
                    elif "potato" in g_ans:
                        selected_crop = "potato"
                except Exception:
                    pass

            # फॉलबॅक लॉजिक (जर Gemini की नसेल तर कापूस आणि सोयाबीनला आधी प्राधान्य द्या)
            if not selected_crop:
                if conf_c > 0.55:
                    selected_crop = "cotton"
                elif idx_s in [0, 1] and conf_s > 0.65:
                    selected_crop = "soybean"
                elif (idx_p in [0, 1] and conf_p > 0.92) or (lesion_ratio > 0.04 and conf_p > conf_s):
                    selected_crop = "potato"
                elif conf_c >= conf_p and conf_c >= conf_s:
                    selected_crop = "cotton"
                elif conf_s >= conf_p:
                    selected_crop = "soybean"
                else:
