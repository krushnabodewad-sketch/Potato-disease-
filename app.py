...
SOYBEAN_CLASSES = ['Soybean Caterpillar Damage (सोयाबीन अळी प्रादुर्भाव)', 'Soybean Leaf Beetle Damage (सोयाबीन भुंगा प्रादुर्भाव)', 'Soybean Healthy Leaf (निरोगी सोयाबीन पान)']
...
TREATMENTS = {
    ...
    'Potato Healthy Leaf (निरोगी बटाटा पान)': {'crop': 'बटाटा · Potato', 'severity': 'सुरक्षित (Healthy)', 'chem': 'प्रतिबंधक रासायनिक फवारणीची गरज नाही.', 'bio': 'संतुलित सेंद्रिय खताद्वारे मातीचे आरोग्य जपा.', 'd7': 'सूक्ष्मअन्नद्रव्ये (Micronutrients) २ मिली प्रति लिटर द्या.', 'd15': 'नियमित पाणी व्यवस्थापन ठेवावे.'},
...
if uploaded_file is not None and models_ready:
    ...
    if is_oos:
        ...
        if "बटाटा" in crop_mode: sc = "potato"
        elif "कापूस" in crop_mode: sc = "cotton"
        elif "सोयाबीन" in crop_mode: sc = "soybean"
        else:
            if (ip in [0, 1] and cp > 0.85) or (l_rat > 0.03 and cp > cs): sc = "potato"
            elif isoy in [0, 1] and cs > 0.65: sc = "soybean"
            elif cc > 0.60: sc = "cotton"
            elif cp >= cs and cp >= cc: sc = "potato"
            elif cs >= cc: sc = "soybean"
            else: sc = "cotton"
        ...
        with col_r:
            st.markdown(f'<div class="k-card"><b>🩺 निदान टर्मिनल (Diagnostic Terminal)</b><div style="margin:8px 0;"><span class="k-pill k-pill-dark">{c_name}</span><span class="k-pill k-pill-light">{diag}</span></div><span class="{tag_c}">● {s_txt}</span><div class="c-val">{f_conf:.1f}%</div><div style="font-size:12px;color:#64748B;">Confidence Score</div></div>', unsafe_allow_html=True)

            a_txt = f"निदान: {c_name}, {diag}. औषध: {inf['chem']}."
            a_js = json.dumps(a_txt)
            components.html(f'<script>function spk(){{window.speechSynthesis.cancel();var m=new SpeechSynthesisUtterance({a_js});m.lang="mr-IN";window.speechSynthesis.speak(m);}}</script><button onclick="spk()" style="width:100%;background:linear-gradient(135deg,#059669,#10b981);color:#fff;border:none;padding:12px;border-radius:12px;font-weight:700;cursor:pointer;">🔊 ऑडिओ सल्ला ऐका (Listen Audio)</button>', height=54)
